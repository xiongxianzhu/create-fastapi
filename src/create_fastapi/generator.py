...
def resolve_template(
    ...
    if _is_git_url(spec):
        tmp = Path(tempfile.mkdtemp(prefix="create-fastapi-tpl-"))
        try:
...
def _is_included(rel_posix: str, opts: ProjectOptions) -> bool:
    if rel_posix in CELERY_PATHS and not opts.use_celery:
        return False
    if rel_posix in REDIS_PATHS and not opts.use_redis:
        return False
...
def generate_project(opts: ProjectOptions) -> GenerationResult:
    ...
    staging_root = Path(tempfile.mkdtemp(prefix="create-fastapi-"))
    try:
