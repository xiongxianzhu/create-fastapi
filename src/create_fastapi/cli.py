...
from create_fastapi import __version__
...
def _version_callback(value: bool) -> None:
    if value:
        typer.echo(f"create-fastapi {__version__}")
        raise typer.Exit()
...
def main(
    ...
    """生成一个新的 FastAPI 项目骨架：``create-fastapi <name>``。"""
...
def _print_summary(opts: ProjectOptions, result: GenerationResult, template: str | None) -> None:
    ...
    typer.echo("  uv sync")
    typer.echo("  cp .env.example .env   # 修改 SECRET_KEY、DATABASE_URL 等")
    typer.echo("  uv run alembic upgrade head")
    typer.echo("  uv run uvicorn app.main:app --reload")
    if opts.use_celery:
        typer.echo("  uv run celery -A app.celery_app:celery_app worker -l info   # 需本地 Redis")
    if opts.use_docker:
